...
<div class="ym-grid">
    ...
    <div class="ym-g50 ym-gl" style="text-align: right; margin-top: 30px;">
        <div class="ym-gbox">
            <strong> Saldo pendiente antes de extras no adelantados:  $ <?php echo $pendiente_previo; ?> </strong>
        </div>
    </div>
</div>
...
<?php } ?>
<?php $saldo_pendiente = $pendiente_previo + $no_adelantadas - $pagado; ?>
...
<div class="ym-gbox saldo_pendiente">
    ...
    <?php echo $this->Form->hidden('ReservaCobro.pendiente',array('value'=>$pendiente_previo + $no_adelantadas - $pagado)); ?></td>
</div>
...
<div class="ym-gbox">
    <table width="100%">
        ...
        <?php foreach($reserva['ReservaFactura'] as $factura){ ?>
        <tr>
            <td><?php echo $factura['fecha_emision']?></td>
            <td><?php echo $factura['tipo']?> - <?php echo $factura['numero']?></td>
            <td><?php echo $factura['titular']?></td>
            <td align="right">$<?php echo $factura['monto']?></td>
        </tr>
        <?php $factura_total = $factura_total + $factura['monto']; } ?>
        <tr>
...
<div class="ym-grid">
    ...
    <div class="ym-g33 ym-gl"><?php echo $this->Form->input('ReservaFactura.fecha_emision',array('class' => 'datepicker', 'type' => 'text', 'label' => 'Fecha de emision'));?></div>
    <div class="ym-g33 ym-gl"><?php echo $this->Form->input('ReservaFactura.numero');?></div>
</div>
<div class="ym-grid">
    <div class="ym-g33 ym-gl"><?php echo $this->Form->input('ReservaFactura.titular');?></div>
    <div class="ym-g33 ym-gl"><?php echo $this->Form->input('ReservaFactura.monto',array('type' => 'text', 'label' => 'Monto $', 'class' => 'number'));?></div>
    <div class="ym-g33 ym-gr"><span style="margin-top: 15px;" onclick="guardar('<?php echo $this->Html->url('/reserva_facturas/guardar.json', true);?>',$('form').serialize(),{id:'w_reservas',url:'/v2/reservas/index'});"  class="boton agregar"> + agregar </span></div>
</div>
<script>
function addExtra(){
    var pattern = /^(([1-9]\d*))$/;
...
function updateTotal(){
    var result = 0;
    var extra_total = 0;
    result += parseFloat(<?php echo $pendiente_previo - $pagado; ?>);
