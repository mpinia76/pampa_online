 ...
 */
	public function text($width = 72) {
		...
		if (!empty($description)) {
			$out[] = phpString::wrap($description, $width);
			$out[] = '';
		}
		...
		if (!empty($subcommands)) {
			...
			foreach ($subcommands as $command) {
				$out[] = phpString::wrap($command->help($max), array(
					'width' => $width,
		...
		if (!empty($options)) {
			...
			foreach ($options as $option) {
				$out[] = phpString::wrap($option->help($max), array(
					'width' => $width,
					'indent' => str_repeat(' ', $max),
					'indentAt' => 1
				));
		...
		if (!empty($arguments)) {
			...
			foreach ($arguments as $argument) {
				$out[] = phpString::wrap($argument->help($max), array(
					'width' => $width,
					'indent' => str_repeat(' ', $max),
					'indentAt' => 1
				));
		...
		if (!empty($epilog)) {
			$out[] = phpString::wrap($epilog, $width);
			$out[] = '';
		}
		return implode("\n", $out);
	}
