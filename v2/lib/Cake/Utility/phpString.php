 ...
 * Example: `phpString::insert(':name is :age years old.', array('name' => 'Bob', '65'));`
 ...
 * - clean: A boolean or array with instructions for phpString::cleanInsert
 *
 ...
 */
	public static function insert($str, $data, $options = array()) {
		...
		$data = (array)$data;
		if (empty($data)) {
			return ($options['clean']) ? phpString::cleanInsert($str, $options) : $str;
		}
...
/**
 * Cleans up a phpString::insert() formatted string with given $options depending on the 'clean' key in
		...
		switch ($clean['method']) {
			case 'html':
				...
				if ($clean['andText']) {
					...
					$str = phpString::cleanInsert($str, $options);
				}
				break;
			case 'text':
				$clean = array_merge(array(
