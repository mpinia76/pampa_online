 ...
 */
	protected function _scaffoldIndex($params) {
		$this->controller->setLogUsuario($this->controller->name);
                                    $this->layout = 'index';
	...
	protected function _scaffoldForm($action = 'edit') {
		...
		$this->controller->viewVars['scaffoldFields'] = array_merge(
			...
			array_keys($this->ScaffoldModel->hasAndBelongsToMany)
                                    ...
                                    if( (isset($this->request->params['pass'][0]) and $this->request->params['pass'][0] == 'exito') or (isset($this->request->params['pass'][1]) and $this->request->params['pass'][1] == 'exito') ){
                                        $this->controller->set('status','exito');
                                    }
			...
			if ($action == 'edit') {
				if (isset($request->params['pass'][0])) {
					$this->ScaffoldModel->id = $request['pass'][0];
                                                                                          $this->redirect = array('action' => 'edit',$request['pass'][0],'exito');
				}
				if (!$this->ScaffoldModel->exists()) {
					throw new NotFoundException(__d('cake', 'Invalid %s', Inflector::humanize($this->modelKey)));
				}
