 ...
 */
class Model extends phpObject implements CakeEventListener {
			...
			if (!empty($this->id)) {
				...
				if (empty($this->data[$this->alias][$this->primaryKey]) && $isUUID) {
					if (array_key_exists($this->primaryKey, $this->data[$this->alias])) {
						...
						$values[$j] = phpString::uuid();
					} else {
						list($fields[], $values[]) = array($this->primaryKey, phpString::uuid());
					}
				}
		...
		if ($state === 'before') {
			if (empty($query['fields'])) {
				...
				if (!is_array($query['fields'])) {
					$query['fields'] = phpString::tokenize($query['fields']);
				}
