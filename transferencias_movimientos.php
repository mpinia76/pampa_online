...
$sql = "INSERT INTO usuario_log (usuario_id,nombre,accion,ip)
			VALUES ('".$_SESSION['userid']."','".$_SESSION['usernombre']."','Transferencias a debitar','".getRealIP()."')";
mysql_query($sql);
$date = date('Y-m-d');
...
if($totalAuditoria == 1) {
    ...
    $sql_update = "UPDATE usuario_auditoria SET last = now(), interaccion='Transferencias a debitar', segundos = segundos + $elapsed_time_seconds WHERE usuario_id = " . $_SESSION['userid'] . " AND fecha = '$date'";
