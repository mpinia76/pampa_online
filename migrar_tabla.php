...
$sql = "SELECT clientes.email, encuesta.*
	...
	echo $sql;
	$rsTemp = mysql_query($sql, $connectionVillaged);
	//print_r($rsTemp);
	while ($rs = mysql_fetch_array($rsTemp)){
		...
		$rsTemp1 = mysql_query($sql, $connectionTest) ;
		//print_r($rsTemp1);
		$reserva_id=0;
		...
		mysql_query($sql, $connectionTest) ;
		$sql = "SELECT *
			FROM encuesta_respuestas WHERE encuesta_id = ".$rs['id'];
			mysql_select_db($dbnameVillaged, $connectionVillaged);
			echo $sql."<br>";
			$rsTemp2 = mysql_query($sql,$connectionVillaged);
			//print_r($rsTemp);
