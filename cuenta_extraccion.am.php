...
if(isset($_POST['agregar'])){
	...
	$sql_entra = "INSERT INTO caja_movimiento (fecha,origen,caja_id,monto,usuario_id) 
				VALUES 
				('".fechasql($_POST['fecha'])."','desdecuenta_".$_POST['cuenta_id']."','".$_POST['caja_id']."','".$_POST['monto']."',$user_id)";
...
<div class="content">
	...
	<? $sql = "SELECT banco.banco,cuenta_tipo.cuenta_tipo,cuenta.* FROM cuenta INNER JOIN cuenta_tipo ON cuenta.cuenta_tipo_id=cuenta_tipo.id INNER JOIN banco ON cuenta.banco_id=banco.id INNER JOIN usuario_cuenta ON usuario_cuenta.cuenta_id = cuenta.id AND usuario_cuenta.usuario_id = $user_id ORDER BY banco.banco";		$rsTemp = mysql_query($sql); echo mysql_error();
...
<p align="center"><input type="submit" value="Hacer extraccion" name="agregar" /></p>
