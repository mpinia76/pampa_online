...
function getDataFromDB($name_mask,$code_mask,$sort_by,$sort_dir){
                ...
                $sql = "SELECT  * FROM rel_pago_operacion Where 0=0";
                if($nm_mask!='')
                        $sql.= " and forma_pago like '$nm_mask%'";
                if($cd_mask!='')
                        $sql.= " and id like '$cd_mask%'";
                if($sort_dir=='')
                        $sort_dir = "asc";
                if($sort_by!='')
                ...
                if($res){
                        while($row=mysql_fetch_array($res)){
                                print("<row id='".$row['id']."'>");
                                        print("<cell>");
                                        print($row['forma_pago']);//."[".$row['item_id']."]");    
                                        print("</cell>");
                                        print("<cell>");
